document.addEventListener("DOMContentLoaded", () => {
    ...
    const graphTitles = [
        ...
        "Average Word Length (SRS with n = 5)",
        "Average Word Length (SRS with n = 20)"
    ];
    ...
    function updateGraph(method) {
        ...
        const height = MMAPPLETS.SETTINGS.graphSettings.height - 75;
    ...
    function highlightSample(indices) {
        ...
        // highlightedSample.forEach(i => {
        //     const el = document.getElementById("wordToken" + i);
        //     if (el) el.classList.remove("selectedWord");
        // });

        document.querySelectorAll('.selectedWord').forEach(elem => {
            elem.classList.remove("selectedWord");
        })

        highlightedSample = indices;

        highlightedSample.forEach(i => {
            const el = document.getElementById("wordToken" + i);
            if (el) el.classList.add("selectedWord");
        });
    }

    function renderText() {
        const outputDiv = document.getElementById("lyricsContainer");
        const outputDiv2 = document.getElementById("lyricsContainer2");
        const lines = textData.trim().split("\n");

        let tokenCnt = 0;
        let column2Started = false;
        ...
        lines.forEach((line, i) => {
            ...
            tokens.forEach(token => {
                ...
                const inColumn2 = tokenCnt > 143;
                const target = inColumn2 ? outputDiv2 : outputDiv;
                if (inColumn2 && !column2Started && /^\s+$/.test(token)) {
                    return;
                }
                ...
                if (/^[\w']+$/.test(token)) {
                    ...
                    if (inColumn2) column2Started = true;
            ...
            if (i < lines.length - 1) {
                ...
                } else if (column2Started) {
                    outputDiv2.appendChild(br);
                }
            }
        });
    }
