...
MMAPPLETS.UI = {
  ...
  toggleElementDisplay: function (elementId, newVal = -1) {
    ...
    if (typeof elem != null) {
      ...
      console.log(
        "ERROR: Cannot toggle display of nonexistent element of id " +
          elementId,
      );
    }
  },
...
MMAPPLETS.MATH = {
  ...
  findMinAndMaxVals: function (dataList) {
    ...
    if (numericList.length === 0) {
      throw new Error(
        "No valid numeric data found, perhaps it was never filtered?",
      );
    }
...
MMAPPLETS.GRAPHS = {
  ...
  makeScale: function (type, domain, range, options = {}) {
    ...
    switch (type) {
      case "band":
        ...
        break;
      default:
        newScale = d3.scaleLinear().domain(domain).range(range);
  ...
  drawCircles: function (svg, data, xScale, yScale, options = {}) {
    ...
    const radius =
      options.radius || MMAPPLETS.SETTINGS.userSettings.pointRadius;
